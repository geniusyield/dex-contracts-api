...
module GeniusYield.OnChain.DEX.PartialOrderNFTV1_1 (mkPartialOrderNFTV1_1Policy) where
...
import           GeniusYield.OnChain.Plutarch.Utils         (pfindOutputWithNft, pfloor, pisAddressForScript,
                                                             pparseDatum')
...
mkPartialOrderNFTV1_1Policy = plam $ \sh refInputAddr refInputToken mtxOutRef ctx ->
    ...
    checkOutput = plam $
        \sh deposit outputs datums cs tn makerFeeFlatContract makerFeeRatioContract takerFeeFlatContract -> unTermCont $ do
            ...
            pguardC "fees too low" $
                ...
                #&& pfloor # ((prationalFromData # makerFeeRatioContract) #* (Rational.pfromInteger # offeredAmount)) #<= pfromData containedFeeOff
